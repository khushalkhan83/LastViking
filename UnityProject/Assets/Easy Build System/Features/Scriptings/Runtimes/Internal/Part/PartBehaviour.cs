                    ...
                    if (CustomDetections[i] != null)
                    {
                        ...
                        for (int p = 0; p < Parts.Length; p++)
                        {
                            ...
                            if (Part != null)
                            {
                                if (Part != this)
                                {
                                    bool isBuilt = !(Part.CurrentState == StateType.Preview || Part.CurrentState == StateType.Queue);
                                    bool supportsType = CustomDetections[i].CheckType((int)Part.Type);
                                    bool usePhysics = Part.AffectedByPhysics;

                                    if (!usePhysics && (isBuilt && supportsType))
                                    {
                                        Results[i] = true;
                                    }
                                }
