...
//---------------------------------------------------------------------------------------------------------------------
    ...
    public static void CompressFolderToZip(string outPathname, string password, string folderName) {
        ...
        using(var zipStream = new ZipOutputStream(fsOut)) {
            ...
            CompressFolderToZipInternal(folderName, outPathname, zipStream, folderOffset);
...
//---------------------------------------------------------------------------------------------------------------------
    ...
    private static void CompressFolderToZipInternal(string path, string outPathname, ZipOutputStream zipStream, int folderOffset) {
        
        var files = Directory.GetFiles(path, "*.*", SearchOption.TopDirectoryOnly);

        foreach (var filename in files) 
        {
            if(filename == outPathname)
            {
                continue;
            }
