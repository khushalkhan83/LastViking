...
namespace CodeStage.AntiCheat.EditorCode.PropertyDrawers
{
    ...
    internal class ObscuredStringDrawer : ObscuredPropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty prop, GUIContent label)
        {
            ...
            var lText = label.text;
            var iBegin = prop.propertyPath.LastIndexOf('[') + 1;
            if (iBegin > 0)
            {
                var iEnd = prop.propertyPath.LastIndexOf(']');
                var index = "Element " + prop.propertyPath.Substring(iBegin, iEnd - iBegin);
                label.text = index;
            }
