					...
					else
					{
						...
						using (ACTkEditorGUI.Horizontal())
						{
							...
							if (GUILayout.Button("Include manually (see readme.pdf)"))
							{
								...
								var providerPath = string.Empty;
								var providers = SettingsService.FetchSettingsProviders();
								foreach (var settingsProvider in providers)
								{
									if (settingsProvider.name == "Graphics")
									{
										providerPath = settingsProvider.settingsPath;
									}
								}

								if (!string.IsNullOrEmpty(providerPath))
								{
									EditorPrefs.SetString("SettingsWindow_Settings_current_provider", providerPath);
									EditorApplication.ExecuteMenuItem("Edit/Project Settings...");
								}
								else
								{
									Debug.LogError(ACTkEditorGlobalStuff.LogPrefix + 
									               "Couldn't open Graphics settings via settings provider, please report to " + 
									               ACTkEditorGlobalStuff.ReportEmail);
								}
#else
