...
Shader "Skybox/Procedural2" 
{
	Properties
	{
		...
		_AdditiveColor("Additive Color", Color) = (.369, .349, .341, 1)
	...
	SubShader
	{
		...
		Pass 
		{
			...
			#define SKY_GROUND_THRESHOLD 0.1	
			...
			half4 frag(v2f IN) : SV_Target
			{
				...
				y = ray.y / SKY_GROUND_THRESHOLD;
			#elif SKYBOX_SUNDISK == SKYBOX_SUNDISK_SIMPLE
				half3 ray = IN.rayDir.xyz;
				y = ray.y / SKY_GROUND_THRESHOLD;
			#else
				y = IN.skyGroundFactor;
			#endif
				
				// if we did precalculate color in vprog: just do lerp between them
				col = lerp(IN.skyColor, step(y, 0) * IN.groundColor, saturate(y));
				col += _AdditiveColor;
			#if SKYBOX_SUNDISK != SKYBOX_SUNDISK_NONE
				col += step(y, 1) * IN.sunColor * calcSunAttenuation(_WorldSpaceLightPos0.xyz, -ray);
			#endif
