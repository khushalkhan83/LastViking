...
namespace MarchingBytes {
	...
	class Pool {
		...
		public Pool(string poolName, GameObject poolObjectPrefab, int initialCount, bool fixedSize, Transform poolParent) {
		...
		private PoolObject NewObjectInstance() {
			GameObject go = (GameObject)GameObject.Instantiate(poolObjectPrefab, poolParent);
		...
		//o(1)
		public GameObject NextAvailableObject(Vector3 position, Quaternion rotation, Transform paretn) 
		{
			PoolObject po = null;
			...
			if(po != null) {
				...
				result.transform.parent = paretn;
	...
	public class EasyObjectPool : MonoBehaviour {
		...
		private void CreatePools() {
			foreach (PoolInfo currentPoolInfo in poolInfo) {
				...
				Pool pool = new Pool(currentPoolInfo.poolName, currentPoolInfo.prefab, 
				                     currentPoolInfo.poolSize, currentPoolInfo.fixedSize, this.transform);

				
				Log("Creating pool: " + currentPoolInfo.poolName);
				//add to mapping dict
		...
		*/
		public GameObject GetObjectFromPool(string poolName, Vector3 position, Quaternion rotation, Transform paretn) {
			GameObject result = null;
			
			if(poolDictionary.ContainsKey(poolName)) {
				Pool pool = poolDictionary[poolName];
				result = pool.NextAvailableObject(position, rotation, paretn);
				//scenario when no available object is found in pool
				if(result == null) {
					LogWarning("No object available in pool. Consider setting fixedSize to false.: " + poolName);
				}
		...
		public void ReturnObjectToPool(GameObject go) {
			PoolObject po = go.GetComponent<PoolObject>();
			if(po == null) {
				LogWarning("Specified object is not a pooled instance: " + go.name);
			} else {
				if(poolDictionary.ContainsKey(po.poolName)) {
					...
					LogWarning("No pool available with name: " + po.poolName);
				}
			}
		}
