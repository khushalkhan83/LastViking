...
public class UserReportingScript : MonoBehaviour
{
    ...
    public Canvas SubmittingPopup;
      ...
      /// <summary>
    /// Gets or sets the UI shown while start submitting.
    /// </summary>
    ...
    public int clickCountSecret = 0;

    public float lastClickTime;
    public float summaryMinSigns = 5;
    ...
    public void CreateUserReport()
    {
        ...
        DescriptionWarning.SetActive(false);
        ...
        // Take Main UnityUserReporting.CurrentClient
        ...
        UnityUserReporting.CurrentClient.CreateUserReport((br) =>
        {
            ...
            var savesData = CreateSavesData();
            br.Attachments.Add(new UserReportAttachment("Saves", "saves.zip", "text/plain", savesData));
            br.Attachments.Add(new UserReportAttachment("User ID", "UserID.txt", "text/plain", System.Text.Encoding.UTF8.GetBytes(AnalyticsSessionInfo.userId)));
    ...
    private byte[] CreateSavesData()
    {
        string savesPath = ModelsSystem.Instance?._storageModel.SaveRootPath;

        string outPathName = Path.Combine(savesPath, "saves.zip");

        ZipUtility.CompressFolderToZip(outPathName,"", savesPath);

        return File.ReadAllBytes(outPathName);
    }
    ...
    {
        this.SummaryInput.onValueChanged.AddListener(OnSummaryChanged);
    }

    private void OnDisable() 
    {
        this.SummaryInput.onValueChanged.RemoveListener(OnSummaryChanged);
    }

    private void OnSummaryChanged(string summary)
    {
        if(!string.IsNullOrWhiteSpace(this.SummaryInput.text) && this.SummaryInput.text.Length >= summaryMinSigns)
        {
            DescriptionWarning.SetActive(false);
        }
    }
    ...
    public void SubmitUserReport()
    {
        ...
        if(!IsInSilentMode && (string.IsNullOrWhiteSpace(this.SummaryInput.text) || this.SummaryInput.text.Length < summaryMinSigns))
        {
            DescriptionWarning.SetActive(true);
            return;
        }
    ...
    private void Update()
    {
        ...
        if (this.SubmittingStartPopup != null)
        {
            this.SubmittingStartPopup.enabled = this.State == UserReportingState.CreatingUserReport;
        }
