...
namespace Gamekit3D
{
    ...
    public class PlayerController : MonoBehaviour, IMessageReceiver
    {
        ...
        void OnDisable()
        {
            if(m_Damageable != null && m_Damageable.onDamageMessageReceivers != null)
            {
        ...
        [SerializeField] private Button fixedUpdateButton;
        [SerializeField] private Button animationUpdateButton;
        public void SwitchUpdateMethod()
        {
            fixedUpdated = !fixedUpdated;
            if(fixedUpdateButton == null) return;
            fixedUpdateButton.GetComponentInChildren<Text>().text = fixedUpdated ? "Fixed" : "Normal";
        }

        public void SwitchAnimationUpdate()
        {
            physicsAnimation = !physicsAnimation;

            if(animationUpdateButton == null) return;
            animationUpdateButton.GetComponentInChildren<Text>().text = physicsAnimation ? "Physics" : "Normal";

            m_Animator.updateMode = physicsAnimation ? AnimatorUpdateMode.AnimatePhysics : AnimatorUpdateMode.Normal;
        }

        private void FixedUpdate() {
            if(!fixedUpdated) return;
        ...
        void Loop()
        {
            ...
            if (m_Input.AttackTap && CanAttack())
        ...
        void TimeoutToIdle()
        {
            bool inputDetected = IsMoveInput || m_Input.AttackTap || m_Input.JumpInput;
        ...
        {
            return canAttack && PlayerEventHandler.CanUseWeapon;
        }
    }
}