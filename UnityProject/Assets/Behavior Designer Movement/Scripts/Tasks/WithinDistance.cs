...
namespace BehaviorDesigner.Runtime.Tasks.Movement
{
    ...
    public class WithinDistance : Conditional
    {
        ...
        {
            if (transform == null || (objects == null && targetObject.Value == null))
            ...
            if (targetObject.Value != null)
            {
                if (IsWithinDistance(targetObject.Value))
                {
        ...
        private bool IsWithinDistance(GameObject gameObject)
        {
            ...
            if (Vector3.SqrMagnitude(direction) < sqrMagnitude)
            {
                ...
                {
                    if (MovementUtility.LineOfSight(transform, offset.Value, gameObject, targetOffset.Value, usePhysics2D, ignoreLayerMask.value))
                    {
